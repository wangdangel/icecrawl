...
let scrapesTable,
  scrapesPaginationPrev,
  scrapesPaginationNext,
  scrapesPaginationCurrent,
  scrapesPaginationShowing,
  scrapesPaginationTotal;
let jobsTable,
  jobsPaginationPrev,
  jobsPaginationNext,
  jobsPaginationCurrent,
  jobsPaginationShowing,
  jobsPaginationTotal,
  statusFilter;
let crawlJobsTable,
  crawlJobsPaginationPrev,
  crawlJobsPaginationNext,
  crawlJobsPaginationCurrent,
  crawlJobsPaginationShowing,
  crawlJobsPaginationTotal,
  crawlStatusFilter;
...
function setupEventListeners() {
  ...
  navScrapes.addEventListener('click', e => {
    e.preventDefault();
    switchPage('scrapes');
  });
  ...
  navCrawlJobs.addEventListener('click', e => {
    e.preventDefault();
    switchPage('crawl-jobs');
  });
  navTransformers.addEventListener('click', e => {
    e.preventDefault();
  ...
  crawlJobsPaginationPrev.addEventListener('click', () => {
    if (state.crawlJobs.page > 1) {
      state.crawlJobs.page--;
      loadCrawlJobs();
    }
  });
  crawlJobsPaginationNext.addEventListener('click', () => {
    if (state.crawlJobs.page < Math.ceil(state.crawlJobs.total / state.crawlJobs.limit)) {
      state.crawlJobs.page++;
      loadCrawlJobs();
    }
  });
  ...
  crawlStatusFilter.addEventListener('change', () => {
    state.crawlJobs.page = 1;
    loadCrawlJobs();
  });
}
...
async function loadDashboardStats() {
  ...
  try {
    ...
    if (result.status === 'success') {
      ...
      document.getElementById('stat-total-scrapes').textContent = Number(
        result.data.totalScrapes || 0,
      );
      document.getElementById('stat-favorites').textContent = Number(
        result.data.totalFavorites || 0,
      );
...
function renderJobsTable(jobs) {
  ...
  if (jobs.length === 0) {
    ...
    jobsTable
      ...
      .forEach(b => b.addEventListener('click', handleJobRetry));
    jobsTable
      .querySelectorAll('[data-action="delete-job"]')
      .forEach(b => b.addEventListener('click', handleJobDelete));
  }
}
...
function renderCrawlJobsTable(jobs) {
  ...
  if (jobs.length === 0) {
    ...
    crawlJobsTable
      .querySelectorAll('[data-action="view-crawl"]')
      .forEach(b => b.addEventListener('click', handleCrawlJobView));
    crawlJobsTable
      .querySelectorAll('[data-action="delete-crawl-job"]')
      .forEach(b => b.addEventListener('click', handleCrawlJobDelete));
  }
}
...
function renderTransformers(transformers) {
  ...
  if (transformers.length === 0) {
    ...
    transformersList
      .querySelectorAll('[data-transformer]')
      .forEach(b => b.addEventListener('click', handleTransformerApply));
  }
}
...
function switchPage(page) {
  ...
  document
    .querySelectorAll('#nav-dashboard, #nav-scrapes, #nav-jobs, #nav-crawl-jobs, #nav-transformers')
    .forEach(nav => {
...
function showNewScrapeModal() {
  newScrapeModal.classList.remove('hidden');
}
function hideNewScrapeModal() {
  newScrapeModal.classList.add('hidden');
  newScrapeForm.reset();
}
function showNewCrawlModal() {
  newCrawlModal.classList.remove('hidden');
}
function hideNewCrawlModal() {
  newCrawlModal.classList.add('hidden');
  newCrawlForm.reset();
}
...
async function fetchCrawlJobDetails(jobId) {
  ...
  const result = await response.json();
  if (result.status !== 'success')
    throw new Error(result.message || 'Failed to fetch crawl job details');
  return result.data;
}
...
function showCrawlDetailsModal(details) {
  ...
  modalWindow.document.write(
    `<html><head><title>Crawl Job Details</title></head><body style="font-family:sans-serif;">${content}</body></html>`,
  );
...
function renderPagination(type, dataState) {
  ...
  document.getElementById(`${type}-pagination-showing`).textContent =
