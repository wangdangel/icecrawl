...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function openReader(jobId) {
    readerContent.innerHTML =
      '<div class="text-center py-8"><i class="fas fa-spinner fa-spin text-2xl text-indigo-600"></i></div>';
    ...
    try {
      ...
      if (!posts.length) {
        readerContent.innerHTML =
          '<div class="text-center text-gray-500">No posts found for this project.</div>';
        return;
      }
      ...
      readerContent.innerHTML =
        '<div class="text-center text-gray-500">Failed to load posts.</div>';
    }
  }
  ...
  const forumJobModal =
    document.getElementById('forum-job-modal') || document.getElementById('new-forum-job-modal');
  const forumJobForm = forumJobModal ? forumJobModal.querySelector('form') : null;
  const forumJobSubmitBtn = forumJobModal
    ? forumJobModal.querySelector('button[type="submit"]')
    : null;
  ...
  if (forumJobForm && forumJobSubmitBtn) {
    forumJobForm.addEventListener('submit', async function (e) {
      ...
      const title =
        'Forum Job: ' +
        (document.getElementById('forum-start-url').value || '')
          .replace(/^https?:\/\//, '')
          .split(/[/?#]/)[0];
      ...
      const filePath =
        output === 'sqlite' ? document.getElementById('forum-db-path').value.trim() : undefined;
      ...
      const useCookies = document.getElementById('forum-use-cookies')?.checked || false;
      const cookieString = document.getElementById('forum-cookie-string')?.value.trim() || '';
