  ...
  static async scrapeWithBrowser(url: string, options: {
    ...
    try {
      ...
      if (options.browserType === 'mobile') {
        // Mobile emulation
        await page.setUserAgent('Mozilla/5.0 (Linux; Android 12; Pixel 6 Pro) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.6099.234 Mobile Safari/537.36 Brave/1.62.162');
        ...
        await page.evaluateOnNewDocument(() => {
          ...
          Object.defineProperty(navigator, 'plugins', {
            get: () => [
              ...
              { name: 'Brave Wallet', filename: 'brave-wallet', description: 'Brave Wallet Extension' },
              { name: 'Widevine Content Decryption Module', filename: 'widevinecdmadapter.dll', description: 'Enables Widevine licenses for HTML audio/video' },
              { name: 'Native Client', filename: 'internal-nacl-plugin', description: '' }
            ]
          });
        });
        await page.setCookie(
          { name: 'CONSENT', value: 'YES+1', domain: '.google.com', path: '/', expires: Math.floor(Date.now() / 1000) + 31536000 },
          { name: 'BraveRewards', value: 'enabled', domain: '.brave.com', path: '/', expires: Math.floor(Date.now() / 1000) + 31536000 },
          { name: 'sessionid', value: 'fake-session-id-123', domain: '.example.com', path: '/', expires: Math.floor(Date.now() / 1000) + 31536000 }
        );
      } else {
        // Desktop emulation (default)
        await page.setUserAgent('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.6099.234 Safari/537.36 Brave/1.62.162');
