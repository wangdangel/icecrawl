...
describe('Crawler', () => {
  ...
  const mockScrapedDataPage1: ScrapedData = {
    ...
    title: 'Page 1',
    content:
      '<html><body><a href="/page2">Link 1</a> <a href="http://sub.example.com/page3">Subdomain Link</a> <a href="http://other.com">External Link</a> <a href="#fragment">Fragment Link</a></body></html>',
    metadata: {},
  ...
  const mockScrapedDataPage3: ScrapedData = {
    // Subdomain page
  ...
  beforeEach(() => {
    ...
    mockedMarkdownService.convertHtmlToMarkdown.mockImplementation(
      (html, url) => `markdown: ${url}`,
    ); // Include URL for easier verification
  ...
  it('should run crawl respecting maxDepth and strict domainScope', async () => {
    ...
    expect(mockedScrapeUrl).not.toHaveBeenCalledWith(
      'http://sub.example.com/page3',
      expect.any(Object),
    ); // Out of scope
    ...
    expect(mockedPrismaScrapedPage.upsert).toHaveBeenCalledWith(
      expect.objectContaining({ where: { url: 'http://example.com/page1' } }),
    );
    expect(mockedPrismaScrapedPage.upsert).toHaveBeenCalledWith(
      expect.objectContaining({ where: { url: 'http://example.com/page1' } }),
    );
    expect(mockedPrismaScrapedPage.upsert).toHaveBeenCalledWith(
      expect.objectContaining({ where: { url: 'http://example.com/page2' } }),
    );
    ...
    expect(mockedPrismaCrawlJob.update).toHaveBeenLastCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ status: 'completed', processedUrls: 2, foundUrls: 2 }),
      }),
    ); // page1, page2 found
  });
  ...
  it('should handle different domain scopes (subdomains)', async () => {
    ...
    const jobDataSubdomain = {
      ...mockJobData,
      startUrl: 'http://www.example.com/page1',
      options: JSON.stringify({ maxDepth: 1, domainScope: 'subdomains' }),
    };
    mockedScrapeUrl.mockImplementation(async url => {
      // Adjust mock for new start URL
      if (url === 'http://www.example.com/page1')
        return {
          ...mockScrapedDataPage1,
          url: 'http://www.example.com/page1',
          content:
            '<html><body><a href="http://sub.example.com/page3">Sub</a> <a href="http://example.com/page2">Parent</a></body></html>',
        };
      if (url === 'http://sub.example.com/page3') return mockScrapedDataPage3;
      throw new Error(`Mock not implemented for URL: ${url}`);
    });
  ...
  it('should handle different domain scopes (parent_subdomains)', async () => {
    ...
    const jobDataParentSubdomain = {
      ...mockJobData,
      startUrl: 'http://www.example.com/page1',
      options: JSON.stringify({ maxDepth: 1, domainScope: 'parent_subdomains' }),
    };
    mockedScrapeUrl.mockImplementation(async url => {
      // Adjust mock for new start URL
      if (url === 'http://www.example.com/page1')
        return {
          ...
          url: 'http://www.example.com/page1',
          content:
            '<html><body><a href="http://sub.example.com/page3">Sub</a> <a href="http://example.com/page2">Parent</a> <a href="/page4">Rel</a></body></html>',
        };
      if (url === 'http://sub.example.com/page3') return mockScrapedDataPage3;
      if (url === 'http://example.com/page2') return mockScrapedDataPage2; // Parent domain page
      if (url === 'http://www.example.com/page4')
        return { ...mockScrapedDataPage1, url: 'http://www.example.com/page4', content: '' }; // Relative link page
      throw new Error(`Mock not implemented for URL: ${url}`);
    });
    ...
    expect(mockedPrismaCrawlJob.update).toHaveBeenLastCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ status: 'completed', processedUrls: 4, foundUrls: 4 }),
      }),
    );
  });
  ...
  it('should handle crawl failure and retry', async () => {
    const jobDataFail = {
      ...mockJobData,
      startUrl: 'http://fail.com/page',
      options: JSON.stringify({ maxDepth: 0, domainScope: 'none' }),
    };
    ...
    mockedScrapeUrl.mockRejectedValueOnce(new Error('Initial fail')).mockResolvedValueOnce({
    ...
    expect(mockedPrismaCrawlJob.update).toHaveBeenLastCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ status: 'completed', processedUrls: 1, failedUrls: '[]' }),
      }),
    );
  });
    ...
    expect(mockedPrismaCrawlJob.update).toHaveBeenLastCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          ...
          failedUrls: JSON.stringify(['http://fail.com/page']),
        }),
      }),
    );
  });
  ...
  it('should correctly normalize URLs and avoid duplicates', async () => {
    const jobDataNormalize = {
      ...mockJobData,
      startUrl: 'http://example.com/path/',
      options: JSON.stringify({ maxDepth: 1, domainScope: 'strict' }),
    };
    const page1Content =
      '<html><body><a href="../path/page2">Rel Link</a> <a href="page2#section">Fragment Link</a> <a href="/page2">Absolute Path</a></body></html>';
    // Reset mocks
    jest.clearAllMocks();
    mockedPrismaCrawlJob.update.mockResolvedValue({} as any);
    mockedPrismaScrapedPage.upsert.mockResolvedValue({} as any);
    ...
    mockedScrapeUrl.mockImplementation(async url => {
      if (url === 'http://example.com/path/')
        return { ...mockScrapedDataPage1, url: 'http://example.com/path/', content: page1Content };
      if (url === 'http://example.com/page2') return mockScrapedDataPage2;
      throw new Error(`Mock not implemented for URL: ${url}`);
    });
    ...
    // NOTE: Expecting 2 calls due to mock concurrency, but visited check should prevent redundant processing.
    expect(mockedScrapeUrl).toHaveBeenCalledTimes(2);
