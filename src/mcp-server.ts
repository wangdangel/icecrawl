        ...
        const toolsList = [
          {
            name: 'scrape_url',
            description:
              'Fetches, scrapes, and optionally captures HTML/screenshot from a single URL.',
            inputSchema: {
              type: 'object',
              properties: {
            ...
            inputSchema: {
              ...
              properties: {
                ...
                domainScope: {
                  type: 'boolean',
                  default: true,
                  description: 'Restrict crawl to start URL domain.',
                },
                includePatterns: {
                  type: 'array',
                  items: { type: 'string' },
                ...
                useBrowser: {
                  type: 'boolean',
                  default: false,
                  description: 'Use headless browser during crawl.',
                },
                waitForSelector: {
                  type: 'string',
                  description: 'CSS selector to wait for on each page if useBrowser is true.',
                },
      ...
      switch (request.params.name) {
        case 'scrape_url':
          ...
          try {
            ...
            logger.debug('scrape_url processing:', {
              url: args.url,
            ...
            if (
              ...
              outputFormats.includes('markdown') ||
              outputFormats.includes('json')
            ) {
              if (useBrowser) {
            ...
            if (needsScreenshot) {
              if (!useBrowser && !htmlContent) {
                // Ensure browser ran if screenshot needed
                logger.warn(
                  "Screenshot requested but browser wasn't used initially. Running browser now.",
                );
                htmlContent = await BrowserService.scrapeWithBrowser(args.url, {
                  waitForSelector: args.waitForSelector,
                });
            ...
            logger.error({
              message: 'Failed to execute scrape_url',
        ...
        case 'get_crawl_job_result':
          if (!isValidGetCrawlJobResultArgs(request.params.arguments)) {
            throw new McpError(
              ...
              'Invalid arguments for get_crawl_job_result',
            );
          }
          try {
            ...
            logger.error({
              message: 'Failed to get crawl job result',
              jobId: request.params.arguments.jobId,
              error,
            });
