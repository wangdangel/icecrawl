 ...
 */
export const languageDetectorTransformer: ScrapedDataTransformer<ScrapedData & { language: string }> = {
  ...
  transform: async (data: ScrapedData): Promise<ScrapedData & { language: string }> => {
    ...
    if (content.includes('el') && content.includes('la') && content.includes('que') && content.includes('de')) {
      language = 'es'; // Spanish
    } else if (content.includes('le') && content.includes('la') && content.includes('les') && content.includes('et')) {
      language = 'fr'; // French
    } else if (content.includes('der') && content.includes('die') && content.includes('das') && content.includes('und')) {
      language = 'de'; // German
    }
 ...
 */
export const sentimentAnalyzerTransformer: ScrapedDataTransformer<ScrapedData & { sentiment: { score: number; label: string } }> = {
  name: 'sentimentAnalyzer',
  description: 'Analyzes the sentiment of the text content',
  transform: async (data: ScrapedData): Promise<ScrapedData & { sentiment: { score: number; label: string } }> => {
    ...
    const positiveWords = ['good', 'great', 'excellent', 'best', 'amazing', 'wonderful', 'fantastic', 'happy', 'love', 'like'];
    const negativeWords = ['bad', 'worst', 'terrible', 'awful', 'horrible', 'poor', 'sad', 'hate', 'dislike', 'disappointing'];
